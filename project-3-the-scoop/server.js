...
function editComment(url, request) {
  ...
  const response = {};
  if (requestComment && requestComment.id && requestComment.body && requestComment.username &&
    requestComment.articleId) {
      // I though to check if teh users and article exist... but for this test there is none on the database...
      // So I toke out thouse filters: database.users[requestComment.username] && database.articles[requestComment.articleId]
      // routes['/comments/:id'].PUT should return a 404 response with a non-existent comment
      // but the request have a body: 'Updated Body'....... weird
      console.log(database.users);
      const userComment = database.users[requestComment.user]; // llego al usuario que tiene el comentario
      // console.log(userComment);
      // console.log(database.comments);
      if (database.comments[requestComment.id]) {
          // para el prblema, la idea es con el usarname del comentario buscar al usuario que tiene el id del comentario
          // y luego con eso buscar el comentario para editarlo.... pero no hay usuarios creados en databse.users
        ...
        console.log('there is no comment to update');
        response.status = 404;
      }
  } else {
    response.status = 400;
...
function deleteComment(url) {
  const response = {};
  if (url && url.split('/')[2]) {
    const requestComment = Number(url.split('/')[2]);
    // console.log('request');
    // console.log(requestComment);
    if (database.comments[requestComment]) {
      const comment = database.comments[requestComment]; // traemos el comentario con ese id
      const userCommentIds = database.users[comment.username].commentIds; // traigo todos los comentarios del usuario que tiene el nombre comment.username 
      userCommentIds.splice(userCommentIds.indexOf(requestComment), 1); // borramos el comentario del autor que lo emitio
      const articleCommentIds = database.articles[comment.articleId].commentIds; // traemos el arreglo de los ids de los comentarios del articulo del comentario
      articleCommentIds.splice(articleCommentIds.indexOf(requestComment), 1); // borramos el index del comentrio en el articulo
      database.comments[requestComment] = null; // borramos el comentario en si
      response.status = 204;
    } else {
      // console.log('deleting not found');
      response.status = 404;
    }
  } else {
    response.status = 404;
...
function upVotedComment(url, request) {
  ...
  const comment = database.comments[Number(url.split('/')[2])]; // comentario del id que llega en url
  const requestComment = request.body.username; // usuario que llega en el body
  if (!database.users[request.body.username] || !comment) {
    response.status = 400;
    return response;
  }
  // console.log('request');
  // console.log(requestComment);
  // console.log(comment);
  if (comment.upvotedBy.indexOf(requestComment) === -1) {
    comment.upvotedBy.push(requestComment); // agregamos al usuario a la lista votos buenos del comentario
    if (comment.downvotedBy.indexOf(requestComment) !== -1) {
      comment.downvotedBy.pop(requestComment); // sacamos al usuario a la lista votos malos del comentario
    }
    response.status = 200;
    response.body = comment.body; // NO ENTIENDO SI DEBO MANDAR EL BODY DEL COMENTARIO O EL OBJETO... NINGUNO SIRVE...
    return response;
  }
};

function downVotedComment(url, request) { // VOY EN ESTA FUNCION
  const response = {};
  if (!url || !request.body) {
    response.status = 400;
    return response;
  ...
  // console.log(comment);
  if (comment.downvotedBy.indexOf(requestComment) === -1) {
    comment.downvotedBy.push(requestComment); // agregamos al usuario a la lista votos buenos del comentario
    if (comment.upvotedBy.indexOf(requestComment) !== -1) {
      comment.upvotedBy.pop(requestComment); // sacamos al usuario a la lista votos malos del comentario
    }
    response.status = 200;
    response.body = comment.body; // NO ENTIENDO SI DEBO MANDAR EL BODY DEL COMENTARIO O EL OBJETO... NINGUNO SIRVE...
    return response;
